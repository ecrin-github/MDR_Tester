...
public class TestData_100122 : TestData_Base
{
	public override FullStudy? FetchStudyData(string sd_sid)
	{
		if (sd_sid == "RPCEC00000014")
		{
	        ...
	        fs.iec!.Add(new StudyIEC(sd_sid, 1, 1, "seq", "1.", 1, "n.01", "Patients with non-surgical esophagus tumors of epithelial origin in stages III or IV, located in the following portions of the esophagus: "));	       
	        fs.iec.Add(new StudyIEC(sd_sid, 2, 1, "seq", "1-", 2, "n.01.01", "Cervical esophagus: it begins at the lower edge of the cricoid cartilage and ends at the thoracic inlet (suprasternal fossa), 18cm from ICS."));	    
	        fs.iec.Add(new StudyIEC(sd_sid, 3, 1, "seq", "2-", 2, "n.01.01", "Intrathoracic esophagus:"));	    
	        fs.iec.Add(new StudyIEC(sd_sid, 4, 1, "seq", "a)", 3, "n.01.02.01", "Upper thoracic portion: from the thoracic inlet (suprasternal fossa) to the trachea bifurcation, 24cm from ICS."));	    
	        fs.iec.Add(new StudyIEC(sd_sid, 5, 1, "seq", "b)", 3, "n.01.02.02", "Middle thoracic segment: it is approximately half of the esophagus, between the trachea bifurcation and the esophagus-gastric join, approximately 32 cm from ICS."));
	        fs.iec!.Add(new StudyIEC(sd_sid, 6, 1, "seq", "2.", 1, "n.02", "Patients without any treatment who are eligible for radio chemotherapy treatment at the time of inclusion."));	  
	        fs.iec!.Add(new StudyIEC(sd_sid, 7, 1, "seq", "3.", 1, "n.03", "Capacity to understand the study and readiness to sign the informed consent document."));	  
	        fs.iec!.Add(new StudyIEC(sd_sid, 8, 1, "seq", "4.", 1, "n.04", " Patients with measurable lesions, (in, at least, one dimension: greater diameter) with a diameter greater than or equal to 20mm using conventional techniques (CAT, X rays, US) or greater than or equal to 10mm using helical CAT."));	  
	        fs.iec!.Add(new StudyIEC(sd_sid, 9, 1, "seq", "5.", 1, "n.05", "Age > 18 and < 75 years."));	  
	        fs.iec!.Add(new StudyIEC(sd_sid, 10, 1, "seq", "6.", 1, "n.06", "General health condition ECOG < 2 (Karnosfsky > 60%)."));	  
	        fs.iec!.Add(new StudyIEC(sd_sid, 11, 1, "seq", "7.", 1, "n.07", "Life expectancy over 6 months."));	  
	        fs.iec!.Add(new StudyIEC(sd_sid, 12, 1, "seq", "8.", 1, "n.08", "Patients with organs and bone marrow working normally, defined by the following parameters: leucocytes >3,000/ul, hemoglobin >=9 g/L, absolute neutrophil counting >1,500/ul, platelets >100,000/ul, TGP/TGO 60 ml/min/1.73 m2 for patients with creatinine values higher than normal value, as established by the institution."));	  
	        fs.iec!.Add(new StudyIEC(sd_sid, 13, 1, "seq", "9.", 1, "n.09", "Reproductive-age females should have a negative pregnancy test and should use appropriate contraceptive methods such as intrauterine devices, hormonal contraceptives, barrier method or tubal ligation. Males should use contraceptive methods during treatment. "));	  
	        fs.iec!.Add(new StudyIEC(sd_sid, 14, 1, "seq", "10.", 1, "n.10", "Patients who had received appropriate treatment for skin carcinoma or in situ uterus cancer can also be included."));	
	        
	        fs.iec.Add(new StudyIEC(sd_sid, 6, 2, "seq", "1.", 1, "e.01", "Patients with esophageal tumors in the inferior thoracic portion (distal half of the esophagus, between the tracheal bifurcation and the esophagus-gastric join, approximately 40cm from ICS)."));	       
	        fs.iec.Add(new StudyIEC(sd_sid, 7, 2, "seq", "2.", 1, "e.02", "Patients on another product under research."));	    
	        fs.iec.Add(new StudyIEC(sd_sid, 8, 2, "seq", "3.", 1, "e.03", "Patients with allergy history attributed to chemical or biological compounds similar to TheraCIM h-R3 monoclonal antibody or the chemotherapy used in the study."));
	        fs.iec.Add(new StudyIEC(sd_sid, 9, 2, "seq", "4.", 1, "e.04", "Patients previously treated with murine monoclonal antibodies (for example, ior egf/r3)."));	    
	        fs.iec.Add(new StudyIEC(sd_sid, 10, 2, "seq", "5.", 1, "e.05", "Patients with uncontrolled intercurrent diseases, including: active infections, symptomatic congestive heart failure, unstable angina pectoris, cardiac arrhythmia, and psychiatric or social diseases restricting compliance with clinical trial requirements."));	    
	        fs.iec.Add(new StudyIEC(sd_sid, 11, 2, "seq", "6.", 1, "e.06", "Breastfeeding should be interrupted after patients are included in the clinical trial, due to potential and unknown adverse events for infants."));	    
	        fs.iec.Add(new StudyIEC(sd_sid, 12, 2, "seq", "7.", 1, "e.07", "Patients with previous malignant neoplasias, except patients who received appropriate treatments for skin carcinoma or in situ uterus carcinoma."));	  
	        fs.iec.Add(new StudyIEC(sd_sid, 12, 2, "seq", "8.", 1, "e.08", "Patients with brain metastasis."));	
